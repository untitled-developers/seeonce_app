...
/// Diagnostic logging. Reconnect logs include peer display names / ids / key
/// hashes, so they must never reach production logcat — DiagnosticLog keeps
/// them in an in-memory, on-device buffer (viewable from the UI) and only
/// mirrors to the console in debug builds.
void _log(String message) {
  final m = message.startsWith('[Reconnect] ')
      ? message.substring('[Reconnect] '.length)
      : message;
  DiagnosticLog.instance.add('Reconnect', m);
}
...
class LocalReconnectService {
  ...
  /// When the current outgoing attempt per target started. Used to refuse to
  /// tear down a handshake that is still in flight: a full reconnect (ICE
  /// gathering on both sides + DTLS + auth) can take longer than the
  /// supervisor's early backoff windows, and restarting the offer each sweep
  /// killed every handshake before its answer arrived — the classic
  /// "stuck on Reconnecting… forever" failure.
  ...
  ///
  /// Failures here are not permanent: every later reconnect attempt re-checks
  /// the key hash and the socket and re-establishes them if needed, so a bad
  /// moment at startup (keys still generating, network down) no longer
  /// disables reconnection until the next app restart.
  Future<void> start({required PeerRepository peerRepository}) async {
    if (_started) return;
    _started = true;
    _peerRepository = peerRepository;

    final haveKeys = await _ensureOwnKeyHash();
    final haveSocket = await _ensureSocket();
    if (!haveKeys || !haveSocket) {
      _log('[Reconnect] start incomplete (keys=$haveKeys, socket=$haveSocket)'
          ' — will retry on the next reconnect attempt');
      return;
    }
  ...
  /// Trigger reconnect for a single offline peer (supervisor sweep or manual
  /// UI button). Self-heals the key hash and UDP socket if either is missing,
  /// e.g. after a failed start() or a socket killed by a network change.
  Future<void> reconnectPeer(Peer peer) async {
    _log(
        '[Reconnect] Reconnect requested for ${peer.displayName}');
    if (!await _ensureOwnKeyHash()) {
      _log('[Reconnect] Own key unavailable — reconnect aborted');
      return;
    }
    if (!await _ensureSocket()) {
      _log('[Reconnect] Socket unavailable — reconnect aborted');
      return;
    }
  ...
  /// Compute our own public-key hash if not done yet. Safe to call repeatedly.
  Future<bool> _ensureOwnKeyHash() async {
    if (_ownKeyHash != null) return true;
    try {
  ...
  /// Bind (or re-bind) the UDP socket. The error/done handlers detect a socket
  /// that the OS killed (e.g. on a network change) and drop our reference so
  /// the next reconnect attempt re-binds instead of sending into a dead socket
  /// forever — previously that state was unrecoverable without an app restart.
  Future<bool> _ensureSocket() async {
    ...
    try {
      ...
      socket.listen(
        _onDatagram,
        onError: (Object e) {
          _log('[Reconnect] ⚠ Socket error: $e — will re-bind on next attempt');
          _dropSocket(socket);
        },
        onDone: () {
          _log('[Reconnect] Socket closed');
          _dropSocket(socket);
        },
      );
      _socket = socket;
  ...
  Future<void> _sendOffer(Peer targetPeer) async {
    ...
    // A handshake for this target is still in flight (gathering ICE, waiting
    // for the answer, or connecting). Restarting it now would close the
    // pending pc and orphan the answer the peer is about to send — the main
    // cause of reconnects never completing. Let it finish or age out.
    final since = _pendingSince[toPeerKeyHash];
    if (since != null &&
        DateTime.now().difference(since) < _attemptTimeout) {
      _log('[Reconnect] Attempt for ${targetPeer.displayName} already in '
          'flight (${DateTime.now().difference(since).inSeconds}s old) — '
          'not restarting it');
      return;
    }
    _pendingSince[toPeerKeyHash] = DateTime.now();
    ...
    try {
      ...
      dc.onDataChannelState = (state) {
        ...
        if (state == RTCDataChannelState.RTCDataChannelOpen) {
          ...
          _pendingSince.remove(toPeerKeyHash);
  ...
  Future<void> _handleIncomingOffer(
    ...
    if (_pendingByTarget.containsKey(fromKeyHash)) {
      ...
      _pendingSince.remove(fromKeyHash);
      try {
        await ours?.close();
      } catch (e) {
  ...
  void _send(Map<String, dynamic> payload, InternetAddress address, int port) {
    ...
    if (socket == null) {
      _log('[Reconnect] ⚠ UDP socket not bound — dropping '
          '${payload['type']} to ${address.address}');
      return;
    }
    try {
      ...
      if (sent == 0) {
        // The OS refused the datagram (typically no usable network route).
        _log('[Reconnect] ⚠ UDP send of ${payload['type']} to '
            '${address.address} returned 0 — packet not sent');
      }
    } catch (e, st) {
