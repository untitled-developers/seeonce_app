...
class ConnectionSupervisor {
  ...
  void resumeNow() {
    DiagnosticLog.instance.add('Supervisor', 'Resume sweep — backoff cleared');
    _backoff.clear();
    if (_running) superviseOnce(DateTime.now());
  }
  ...
  Future<void> superviseOnce(DateTime now) async {
    // The timer, pool change events (a reconnect succeeding triggers one), and
    // resume can all fire while a previous pass is still awaiting a reconnect;
    // without this guard sweeps overlap, double-counting backoff attempts and
    // re-offering mid-handshake.
    if (_sweeping) return;
    _sweeping = true;
    try {
      ...
      for (final p in peers) {
        ...
        DiagnosticLog.instance.add('Supervisor',
            'Reconnect attempt $attempt for ${p.displayName} '
            '(next retry in ${backoffFor(attempt).inSeconds}s)');
        try {
          await _reconnect(p);
        } catch (e) {
          DiagnosticLog.instance
              .add('Supervisor', 'Reconnect ${p.displayName} failed: $e');
        }
      }
    } finally {
      _sweeping = false;
